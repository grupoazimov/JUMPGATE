...
int main(int, char**)
{
    ...
    g_single_instance_mutex = CreateMutexW(nullptr, TRUE, L"Local\\AZIMOV_JUMPGATE_SINGLE_INSTANCE");
    if (!g_single_instance_mutex || GetLastError() == ERROR_ALREADY_EXISTS)
    {
        MessageBoxW(nullptr, L"O AZIMOV JUMPGATE já está em execução.", L"AZIMOV JUMPGATE", MB_ICONINFORMATION | MB_OK);
    ...
    {
        const auto connection = core::connection_gate::check();
        if (connection.state == core::connection_gate::RemoteState::NewVersionAvailable)
        {
            MessageBoxW(nullptr, L"Uma nova versão do AZIMOV JUMPGATE foi detectada.\n\nVocê pode continuar usando esta versão por enquanto. Aperte OK para abrir o programa.", L"AZIMOV JUMPGATE", MB_ICONINFORMATION | MB_OK);
        }
        else if (connection.state == core::connection_gate::RemoteState::VersionEnded)
        {
            MessageBoxW(nullptr, L"Esta versão do AZIMOV JUMPGATE foi encerrada.\n\nAperte OK para abrir a página de downloads e baixar uma versão nova.", L"AZIMOV JUMPGATE", MB_ICONWARNING | MB_OK);
            ShellExecuteW(nullptr, L"open", L"https://azimovesports.com/jumpgate/downloads", nullptr, nullptr, SW_SHOWNORMAL);
            return 0;
        }
        else if (connection.state == core::connection_gate::RemoteState::ServerBlocked)
        {
            MessageBoxW(nullptr, L"O acesso ao AZIMOV JUMPGATE foi bloqueado pelo servidor.", L"AZIMOV JUMPGATE", MB_ICONERROR | MB_OK);
            return 0;
        }
        else if (connection.state == core::connection_gate::RemoteState::Unavailable)
        {
            MessageBoxW(nullptr, L"Não foi possível conectar ao servidor do AZIMOV JUMPGATE.\n\nO programa continuará em modo sem conexão com o servidor.", L"AZIMOV JUMPGATE", MB_ICONWARNING | MB_OK);
        }
        else if (connection.state != core::connection_gate::RemoteState::Allowed)
        {
            MessageBoxW(nullptr, L"Não foi possível validar a conexão do AZIMOV JUMPGATE.", L"AZIMOV JUMPGATE", MB_ICONERROR | MB_OK);
            return 0;
        }
    }
    ...
    WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"AZIMOV JUMPGATE", nullptr };
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"AZIMOV JUMPGATE", WS_OVERLAPPEDWINDOW | WS_MINIMIZEBOX, 200, 200, 90, 90, nullptr, nullptr, wc.hInstance, nullptr);
    g_hwnd = hwnd;
    ...
    */
    ...
    io.IniFilename = NULL;
    /*//{
        std::filesystem::path path = std::filesystem::temp_directory_path().append("jumpgate");
        if (!std::filesystem::is_directory(path) || !std::filesystem::exists(path)) {
            std::filesystem::create_directory(path);
        }

        path.append("jumpgate.ini");
