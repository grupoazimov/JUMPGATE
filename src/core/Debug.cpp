...
void Debug::_imgui_debug_status_text(bool happy, std::string text)
{
    ...
    {
        ImGui::TextColored(ImVec4{ 1, 0, 0, 1 }, "(falha)");
    }
...
void Debug::render(/* bool* p_open */)
{
...

    {
        ...
        ImGui::Begin("depuração", NULL, window_flags);
        ImGui::SetWindowSize(ImVec2(432, 400), ImGuiCond_Once);
        ImGui::SetWindowPos(ImVec2(800, 600), ImGuiCond_Once);

        ImGui::SeparatorText("demonstração");
        // ImGui::ShowDemoWindow adds ~0.5mb to exe
        if (ImGui::Button("demonstração")) demo_window_open = true;
        if (demo_window_open)
            ImGui::ShowDemoWindow(&demo_window_open);

        {
            ImGui::SliderFloat("opacidade", &(ImGui::GetStyle().Alpha), 0.4f, 1.0f, "%.2f");
        }
