...
namespace util::win_network {
	...
	NetworkInformation queryNetwork() {
...

        {
            ...
            if (FAILED(pNetworkListManager.CoCreateInstance(CLSID_NetworkListManager))) throw std::runtime_error("falha ao criar instância de rede do Windows");
            
            /* network iterator */
            CComPtr<IEnumNetworks> pEnumerator;
            if (FAILED(pNetworkListManager->GetNetworks(NLM_ENUM_NETWORK_CONNECTED, &pEnumerator))) throw std::runtime_error("falha ao consultar redes conectadas");

            /* network item */
            CComPtr<IEnumVARIANT> pVariant;
            if (FAILED(pEnumerator.QueryInterface(&pVariant))) throw std::runtime_error("falha ao consultar enumerador de redes");
            ...
            for (CComVariant var; pVariant->Next(1, &var, nullptr) == S_OK; var.Clear())
            {
                ...
                if (FAILED(var.ChangeType(VT_DISPATCH))) throw std::runtime_error("falha ao alterar tipo de despacho");
                if (FAILED(V_DISPATCH(&var)->QueryInterface(IID_PPV_ARGS(&pINetwork)))) throw std::runtime_error("falha ao consultar rede");
                ...
                if (FAILED(pINetwork->GetCategory(&network_cat))) throw std::runtime_error("falha ao consultar categoria da rede");
                ...
                else throw std::runtime_error("categoria de rede desconhecida detectada");
            }
