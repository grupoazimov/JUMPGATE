...
namespace util::win_firewall {
    ...
    void forFirewallRulesInGroup(std::string _target_groupName, std::function<void(const CComPtr<INetFwRule>&, const CComPtr<INetFwRules>&)> predicate)
    {
        ...
        if (FAILED(hr))
        {
            wprintf(L"Falha em CoCreateInstance: 0x%08lx\n", hr);
            return;
        }
        ...
        if (FAILED(hr))
        {
            wprintf(L"Falha em get_Rules: 0x%08lx\n", hr);
            return;
        }
        ...
        if (FAILED(hr))
        {
            wprintf(L"Falha em get__NewEnum: 0x%08lx\n", hr);
            return;
        }
        ...
        if (FAILED(hr))
        {
            wprintf(L"get__NewEnum não produziu IEnumVariant: 0x%08lx\n", hr);
            return;
        }
