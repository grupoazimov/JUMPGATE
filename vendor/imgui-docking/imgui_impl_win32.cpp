...
static bool ImGui_ImplWin32_IsJumpgateMinimizeButtonHit(HWND hWnd, LPARAM lParam)
{
    wchar_t title[64] = {};
    ::GetWindowTextW(hWnd, title, IM_ARRAYSIZE(title));
    if (::lstrcmpW(title, L"AZIMOV JUMPGATE") != 0)
        return false;
    ...
    return x >= client_rect.right - 70 && x <= client_rect.right - 38 && y >= 1 && y <= 31;
}

static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    const bool jumpgate_minimize_hit = msg == WM_LBUTTONUP && ImGui_ImplWin32_IsJumpgateMinimizeButtonHit(hWnd, lParam);

    if (ImGui_ImplWin32_WndProcHandler(hWnd, msg, wParam, lParam))
    {
        if (jumpgate_minimize_hit)
        {
